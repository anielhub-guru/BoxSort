...
func _input(event):
	if event is InputEventMouseMotion and dragging:
		dragged_item.follow_mouse(event.position)
...
func end_drag(pos):
	...
	if start_y != end_y or abs(end_x - start_x) != 1:
...
func attempt_swap(item1, item2, x1, y1, x2, y2):
	...
	if check_for_matches():
		...
		grid_data[x1][y1] = item1
		grid_data[x2][y2] = item2
		create_tween().tween_property(item1, "position", pos1, 0.2)
		create_tween().tween_property(item2, "position", pos2, 0.2)
