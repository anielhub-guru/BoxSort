...
func _ready():
	...
	var next_button = get_node_or_null("../../../UI/VBoxContainer/NextLevelButton")
	var restart_button = get_node_or_null("../../../UI/VBoxContainer/RestartButton")
	if next_button != null:
		next_button.visible = false
		next_button.pressed.connect(_on_NextLevelButton_pressed)
...
func load_level(level_number: int) -> void:
	# Load level from JSON file and apply settings
	...
	if result.error != OK:
		print("Error parsing levels.json: %s" % (result.error_string))
		return

	var levels_array = result.result
	var chosen_level = null
	for level_data in levels_array:
		if level_data.has("level_number") and level_data.level_number == level_number:
			chosen_level = level_data
			break

	if chosen_level == null:
		print("Level number %d not found in levels.json" % level_number)
		return

	# Apply level properties
	if chosen_level.has("grid_size"):
		grid_width = chosen_level.grid_size.rows
		grid_height = chosen_level.grid_size.cols

	if chosen_level.has("level_goals"):
		set_custom_level_goals(chosen_level.level_goals, color_textures)

	if chosen_level.has("level_goals") and chosen_level.level_goals.has("time_limit_seconds"):
		time_limit = chosen_level.level_goals.time_limit_seconds
...
func toggle_ui_buttons(enable: bool) -> void:
	var next_button = get_node_or_null("../../../UI/VBoxContainer/NextLevelButton")
	var restart_button = get_node_or_null("../../../UI/VBoxContainer/RestartButton")
	if next_button != null:
		next_button.visible = enable and is_level_complete
	if restart_button != null:
		restart_button.visible = enable and is_game_over
